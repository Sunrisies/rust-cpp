...
fn main() {
    ...
    let cpp_result =
        ffi::run_planning(&map, &pyg, &mut cpp_obs, &params).expect("C++ planning should succeed");
    let rust_result =
        plan(&map, &pyg, &mut rust_obs, &params).expect("Rust planning should succeed");

    terminate();

    eprintln!(
        "C++:  wpCnt={} yaw={:.10} path={:.10} area={:.10}",
        cpp_result.waypoint_count, cpp_result.yaw, cpp_result.path_length, cpp_result.coverage_area
    );
    eprintln!(
        "Rust: wpCnt={} yaw={:.10} path={:.10} area={:.10}",
        rust_result.waypoint_count,
        rust_result.yaw,
        rust_result.path_length,
        rust_result.coverage_area
    );
    eprintln!();
            ...
            for j in 0..i + 3.min(min_n) {
                ...
                if j < min_n {
                    ...
                    eprintln!(
                        "  wp[{}]: C++=({:.9},{:.9},{}) Rust=({:.9},{:.9},{}){}",
                        j,
                        cpp_result.waypoints[b],
                        cpp_result.waypoints[b + 1],
                        cpp_result.waypoints[b + 2] as i32,
                        rust_result.waypoints[b],
                        rust_result.waypoints[b + 1],
                        rust_result.waypoints[b + 2] as i32,
                        flag
                    );
                }
            }
    ...
    if cpp_result.waypoint_count > rust_result.waypoint_count {
        eprintln!(
            "Rust missing {} trailing waypoints:",
            cpp_result.waypoint_count - rust_result.waypoint_count
        );
        for i in rust_result.waypoint_count..cpp_result.waypoint_count {
            ...
            eprintln!(
                "  C++ wp[{}]: ({:.9},{:.9},{})",
                i,
                cpp_result.waypoints[base],
                cpp_result.waypoints[base + 1],
                cpp_result.waypoints[base + 2] as i32
            );
        }
    } else if rust_result.waypoint_count > cpp_result.waypoint_count {
        eprintln!(
            "Rust extra {} trailing waypoints:",
            rust_result.waypoint_count - cpp_result.waypoint_count
        );
        for i in cpp_result.waypoint_count..rust_result.waypoint_count {
            ...
            eprintln!(
                "  Rust wp[{}]: ({:.9},{:.9},{})",
                i,
                rust_result.waypoints[base],
                rust_result.waypoints[base + 1],
                rust_result.waypoints[base + 2] as i32
            );
        }
    }
}
